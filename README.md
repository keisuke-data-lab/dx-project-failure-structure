...
> * **Target Audience:** CIO, DX推進責任者, PMO, 内部監査担当
> * **Ideal Users:** CIOs, PMO leads, Internal Auditors, DX Strategy Teams
...
> 
> `similarity_score = max(0, (1 - (distance / 20)) * 100)`
...
**図解：リスクDNAの照合 (Risk DNA Matching)**

* **Blue Line (Target):** 診断対象プロジェクトのスコア形状。（本データはダミーデータ）
* **Red Line (Reference):** 構造的に最も類似した過去の失敗事例（ここでは `SOFTIC_021_Kyushuuya`）。
* **Insight:**
    青線が赤線（過去の失敗）の形状をトレースしている場合、そのプロジェクトは**「同じ死因」**を抱えています。
    上記の例では、Supply Chain（ベンダー体制）は健全ですが、**Client Immaturity（発注者側の準備不足）が突出しており、これが「九州屋事件」の失敗パターンと合致**しています。
    → *推奨アクション：開発停止と要件定義のやり直し*
---
